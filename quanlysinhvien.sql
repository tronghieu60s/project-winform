...
--
-- Máy chủ: 127.0.0.1
-- Thời gian đã tạo: Th1 14, 2021 lúc 09:08 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `createSubject` (IN `id_subjects` VARCHAR(10), IN `subject_name` VARCHAR(100), IN `credit` INT, IN `information` TEXT, IN `id_lecturer` VARCHAR(10), IN `start_day` DATETIME, IN `end_day` DATETIME, IN `id_course` VARCHAR(10), IN `id_faculty` VARCHAR(10))  NO SQL
INSERT INTO `subjects`(`id_subjects`, `subject_name`, `credit`, `information`, `id_lecturer`, `start_day`, `end_day`, `id_course`, `id_faculty`) VALUES (id_subjects, subject_name, credit, information, id_lecturer, start_day, end_day, id_course, id_faculty)$$
...
SELECT * FROM `subjects` LEFT JOIN `courses` ON `courses`.`id_course` = `subjects`.`id_course` LEFT JOIN `faculties` ON `faculties`.`id_faculty` = `subjects`.`id_faculty` LEFT JOIN `users` ON `users`.`id_user` = `subjects`.`id_lecturer` ORDER BY `subjects`.`date` DESC$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `updateSubjectWithId` (IN `v_id_subjects` VARCHAR(10), IN `subject_name` VARCHAR(100), IN `credit` INT, IN `information` TEXT, IN `id_lecturer` VARCHAR(10), IN `start_day` DATETIME, IN `end_day` DATETIME, IN `id_course` VARCHAR(10), IN `id_faculty` VARCHAR(10))  NO SQL
UPDATE `subjects` SET `subject_name`= subject_name,`credit`= credit,`information`= information,`id_lecturer` = id_lecturer ,`start_day`= start_day,`end_day`= end_day,`id_course`= id_course,`id_faculty`= id_faculty
...
CREATE TABLE `subjects` (
  ...
  `id_lecturer` varchar(10) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
...
INSERT INTO `subjects` (`id_subjects`, `subject_name`, `credit`, `information`, `id_lecturer`, `start_day`, `end_day`, `id_course`, `id_faculty`, `date`) VALUES
('MH02', 'Tiếng Hàn Giao Tiếp', 3, 'TDC', 'GV1003', '2021-01-14 00:00:00', '2021-01-14 00:00:00', 'K20', 'FA02', 2147483647),
('MH03', 'Tiếng Anh Giao Tiếp 2', 3, 'TDC', 'GV1000', '2021-01-14 00:00:00', '2021-01-14 00:00:00', 'K20', 'FA01', 2147483647);
...
('GV1000', 'btWDPPNShuv4Zit7WUnw10K77D8=', 'Trần Dần', NULL, NULL, '2021-01-14 14:52:56'),
('GV1001', 'btWDPPNShuv4Zit7WUnw10K77D8=', 'Trần Đức Bo', NULL, NULL, '2021-01-14 14:53:15'),
('GV1002', 'btWDPPNShuv4Zit7WUnw10K77D8=', 'Trần Thanh Tâm', NULL, NULL, '2021-01-14 14:53:52'),
('GV1003', 'btWDPPNShuv4Zit7WUnw10K77D8=', 'Nancy', NULL, NULL, '2021-01-14 14:54:22'),
...
ALTER TABLE `subjects`
  ...
  ADD CONSTRAINT `subjects_ibfk_1` FOREIGN KEY (`id_lecturer`) REFERENCES `users` (`id_user`) ON DELETE SET NULL;
